...
def is_relevant(query_obj: Dict, result: Dict) -> int:
    ...
    if category == "combined" and rank <= 5:
        if filters:
            ...
            if (
                filters.get("remote_allowed") == True
                and result["remote_allowed"] == False
            ):
                return 0
...
def generate_pseudo_labels():
    ...
    print(
        f"Relevant: {stats['relevant']} ({stats['relevant']/stats['total']*100:.1f}%)"
    )
    print(
        f"Not relevant: {stats['not_relevant']} ({stats['not_relevant']/stats['total']*100:.1f}%)"
    )
